...
/// r50: FurAffinity routes outside links through its /externalurl/ page, and
/// each link says what it is and where it goes.
void main() {
...
&gt;&gt;<a class="auto_link_shortened" href="https://www.furaffinity.net/externalurl/?q=https%3A%2F%2Fwww.patreon.com%2Fsomeartist">Patreon</a>&lt;&lt;
<a href="/externalurl/?q=https://e621.net/posts/5550001">Full <i>animation</i></a>
''';

  test('the links of a description, each once, in order, unwrapped, without profile and script links', () {
    expect(LinkedMediaResolver.linksIn(description, base: 'https://www.furaffinity.net'), [
      ...
      'https://www.patreon.com/someartist',
      'https://e621.net/posts/5550001',
    ]);
    final List<LinkedAnchor> anchors = LinkedMediaResolver.anchorsIn(description, base: 'https://www.furaffinity.net');
    expect(anchors.length, 9);
    expect(anchors[7].text, 'Patreon');
    expect(anchors.last.text, 'Full animation', reason: 'the words of the link, without its markup');
  });

  test("FurAffinity's external link page is unwrapped to where it leads", () {
    expect(
      LinkedMediaResolver.unwrap('https://www.furaffinity.net/externalurl/?q=https%3A%2F%2Fe621.net%2Fposts%2F1'),
      'https://e621.net/posts/1',
    );
    expect(LinkedMediaResolver.unwrap('https://www.furaffinity.net/externalurl/?q=https://x.com/a'), 'https://x.com/a');
    expect(LinkedMediaResolver.unwrap('https://www.furaffinity.net/view/5/'), 'https://www.furaffinity.net/view/5/');
  });

  test('each link resolves to a post on an installed source, a file, or a page, and says where it goes', () {
    LinkedMedia r(String url, [String text = '']) => LinkedMediaResolver.resolve(url, installed, text: text);
    final LinkedMedia post = r('https://e621.net/posts/4011234?q=fox', 'https://e621.net/posts/4011234');
    expect((post.kind, post.booru, post.postId, post.searchTerm), (LinkedMediaKind.sourcePost, e621, '4011234', 'id:4011234'));
    expect(
      (post.title, post.destination, post.badge),
      ('e621 post #4011234', 'Opens in the app · e621', 'In app'),
      reason: 'a link whose words are its own address has no title of its own',
    );
    expect(r('https://e621.net/posts/5550001', 'Full animation').title, 'Full animation');
    ...
    final LinkedMedia video = r('https://files.example.com/anim/final.mp4?token=1', 'link');
    expect(
      (video.kind, video.isImage, video.title, video.destination, video.badge),
      (LinkedMediaKind.media, false, 'final.mp4', 'Plays here · files.example.com', 'Video'),
      reason: '"link" says nothing; the file name does',
    );
    ...
    final LinkedMedia gif = r('https://media.example.org/loop.gif');
    expect((gif.isImage, gif.badge), (true, 'Animation'));

    final LinkedMedia page = r('https://x.com/artist/status/17890', 'x.com');
    expect((page.kind, page.title, page.destination, page.badge), (LinkedMediaKind.page, 'x.com', 'Opens the web page · x.com', 'Web'));
    expect(r('https://www.patreon.com/someartist', '>>Patreon<<').title, 'Patreon');
    expect(r('https://www.patreon.com/someartist').destination, 'Opens the web page · patreon.com');
