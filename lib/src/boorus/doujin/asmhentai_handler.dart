...
class AsmHentaiHandler extends BooruHandler with DoujinListingTagBackfill, DoujinNamespacedTags, CatalogSuggestions {
  ...
  String makeURL(String tags) {
    ...
    // r70: the site's random gallery - one answer, no pages; a category or
    // language chip beside it changes nothing.
    if (DoujinFilters.strip(DoujinFilters.strip(query, 'category'), 'language').trim().toLowerCase() == 'random:') {
      if (page > 1) {
        locked = true;
        return '';
      }
  ...
  FutureOr<List> parseListFromResponse(dynamic response) async {
    ...
    if (DoujinFilters.strip(DoujinFilters.strip(currentTags, 'category'), 'language').trim().toLowerCase() == 'random:') {
      // `/random/` redirects to a gallery: its id is where the site sent us.
      String id = '';
      try {
        id = RegExp(r'/g/(\d+)/').firstMatch(response.realUri.toString())?.group(1) ?? '';
      } catch (_) {}
      if (id.isEmpty) id = RegExp(r'/g/(\d+)/').firstMatch(body)?.group(1) ?? '';
      final item = id.isEmpty ? null : _itemFromGalleryPage(id, body);
      return item == null ? [] : [item];
    }
  ...
  List<Tag> _tagsFrom(dom.Document doc) {
    ...
    for (final a in doc.querySelectorAll('a[href]')) {
      ...
      final Tag tag = namespacedTag(name, namespace, count: _countFrom(a));
  ...
  DoujinFilterSpec get doujinFilters => const DoujinFilterSpec([
    ...
    // r70: the site's two categories, from its navigation.
    DoujinFilterGroup(
      key: 'category',
      label: 'Category',
      options: [DoujinFilterOption('doujinshi', 'Doujinshi'), DoujinFilterOption('manga', 'Manga')],
    ),
  ]);
  ...
  List<MetaTag> availableMetaTags() => [
    ...
    MetaTagWithValues(name: 'Random gallery', keyName: 'random', values: [MetaTagValue(name: 'one', value: '')]),
  ];
