...
class HitomiHandler extends BooruHandler {
  ...
  static String _tagText(String raw) => raw.trim().toLowerCase().replaceAll(' ', '_');
  ...
  @visibleForTesting
  static List<Tag> tagsFromGallery(Map<String, dynamic> gallery) {
    final List<Tag> tags = [];
    final Set<String> seen = {};

    void add(String full, TagType type) {
      if (full.isEmpty || !seen.add(full)) return;
      tags.add(Tag(full, tagType: type));
    }

    void addAll(String key, String field, String namespace, TagType type) {
      for (final entry in gallery[key] as List? ?? const []) {
        if (entry is! Map) continue;
        final String name = _tagText(entry[field]?.toString() ?? '');
        if (name.isEmpty) continue;
        add('$namespace:$name', type);
      }
    }

    addAll('artists', 'artist', 'artist', TagType.artist);
    addAll('groups', 'group', 'circle', TagType.artist);
    addAll('parodys', 'parody', 'parody', TagType.copyright);
    addAll('characters', 'character', 'character', TagType.character);

    for (final entry in gallery['tags'] as List? ?? const []) {
      if (entry is! Map) continue;
      final String name = _tagText(entry['tag']?.toString() ?? '');
      if (name.isEmpty) continue;
      final bool female = entry['female']?.toString().isNotEmpty ?? false;
      final bool male = entry['male']?.toString().isNotEmpty ?? false;
      add(female ? 'female:$name' : (male ? 'male:$name' : name), TagType.none);
    }

    final String type = _tagText(gallery['type']?.toString() ?? '');
    if (type.isNotEmpty) add('type:$type', TagType.meta);
    final String language = _tagText(gallery['language']?.toString() ?? '');
    if (language.isNotEmpty) add('language:$language', TagType.meta);
  ...
  Future<List<BooruItem>> _fetchRecommended(String id, HitomiGg gg) async {
    ...
    for (final tag in source.tagsList) {
      if (tag.fullString.startsWith('artist:')) {
        artist = tag.fullString.substring('artist:'.length);
        break;
      }
    }
  ...
  Future<List<BooruItem>> _candidatesFor(BooruItem source, HitomiGg gg) async {
    ...
    final List<String> terms = [
      for (final tag in source.tagsList)
        if (nozomiTargetFor(tag.fullString) != null) tag.fullString,
    ];
