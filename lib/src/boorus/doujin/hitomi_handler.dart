...
class HitomiHandler extends BooruHandler with DoujinNamespacedTags, CatalogSuggestions {
  ...
  /// A language is a whole-site index (167,000 ids for English today): read
  /// in full when it joins an intersection, or the newest 20,000 of the
  /// language would silently cap every language-filtered search (review).
  static const int languageWindow = 400000;

  /// Language ids read this session, one set per language.
  final Map<String, Set<int>> _languageSets = {};

  /// True for a term that names a whole-site index rather than a tag list.
  @visibleForTesting
  static bool isWholeIndexTerm(String term) {
    final target = nozomiTargetFor(term);
    return target != null && target.area == null && target.tag == 'index';
  }

  Future<Set<int>> _languageIds(String language) async {
    final Set<int>? known = _languageSets[language];
    if (known != null) return known;
    final List<int> ids = await _nozomi((area: null, tag: 'index', language: language), start: 0, count: languageWindow);
    return _languageSets[language] = ids.toSet();
  }
  ...
  /// work here - the language is an index of its own the search intersects
  /// with, and every galleryinfo carries the Japanese title.
  @override
  bool get supportsLanguageFilter => true;
  @override
  bool get supportsTitleLanguage => true;

  bool get _preferJapanese => SourceSettingsHandler.instance.titleLanguage(booru) == 'japanese';

  /// The per-source language joins a query that names none.
  @visibleForTesting
  ...
  /// The two titles in the order asked for; one alone when that is all.
  static String titlesFor(String title, String japanese, {required bool preferJapanese}) {
    final bool both = japanese.isNotEmpty && title.isNotEmpty && japanese != title;
    if (!both) return title.isNotEmpty ? title : japanese;
    return preferJapanese ? '$japanese\n$title' : '$title\n$japanese';
  }
    ...
    if (resolved == null) {
      ...
      if (positive.isEmpty) {
        ...
        for (final term in positive) {
          final String qualified = qualifyTag(term);
          final List<int> ids = isWholeIndexTerm(qualified)
              ? (await _languageIds(nozomiTargetFor(qualified)!.language)).toList()
              : await _idsForTerm(term, start: 0, count: _intersectionWindow);
          if (working == null) {
  ...
  BooruItem? _itemFromGallery(Map<String, dynamic> gallery, HitomiGg gg) {
    ...
    item.description = titlesFor(title, japanese, preferJapanese: _preferJapanese);
