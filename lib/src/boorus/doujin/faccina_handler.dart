...
class FaccinaHandler extends BooruHandler with DoujinNamespacedTags, CatalogSuggestions {
  ...
  /// r70: the Tag builder - the whole taxonomy in one page-data request.
  @override
  late final TagCatalogSource? tagCatalog = FaccinaTagCatalog(this);

  /// faccina's sorts, as the API spells them; the site default needs none.
  ...
  DoujinFilterSpec get doujinFilters => const DoujinFilterSpec([
    DoujinFilterGroup(
      ...
      options: [
        DoujinFilterOption('released', 'Date released'),
        DoujinFilterOption('added', 'Date added'),
        DoujinFilterOption('title', 'Title'),
        DoujinFilterOption('pages', 'Pages'),
  ...
  String qualifyQuery(String tags) {
    ...
    for (final match in RegExp(r'"[^"]*"|\S+').allMatches(tags.trim())) {
      ...
      final String prefix = (token.startsWith('-') || token.startsWith('~')) ? token[0] : '';
  ...
  String makeURL(String tags) {
    ...
    // r70: sort and order are the API's own parameters, never search words.
    final String sort = DoujinFilters.selected(tags, 'sort').lastOrNull ?? '';
    final String order = DoujinFilters.selected(tags, 'order').lastOrNull ?? '';
    final String extra = '${sortParams.containsKey(sort) ? '&sort=${sortParams[sort]}' : ''}${order == 'asc' ? '&order=asc' : ''}';
    // Chips display bare; faccina matches on the namespaced form.
    final String query = translateQuery(qualifyQuery(DoujinFilters.strip(DoujinFilters.strip(tags, 'sort'), 'order')));
    // NB: the parameter is `q`. `search=` is accepted and silently ignored,
    // which would return the whole unfiltered library.
    if (query.isEmpty) return '$_site/api/library?page=$page$extra';
    return '$_site/api/library?q=${Uri.encodeQueryComponent(query)}&page=$page$extra';
  }
  ...
  /// faccina publishes tags already namespaced: `{namespace, name}`. They are
  /// stored bare with the namespace kept beside them, so a chip reads
  /// `wakahi-chan` rather than `artist:wakahi-chan`.
  @visibleForTesting
  List<Tag> tagsFromArchive(Map row) {
    final List<Tag> tags = [];
    ...
    for (final t in row['tags'] as List? ?? []) {
      ...
      final Tag tag = namespacedTag(name, t['namespace']?.toString());
      if (tag.fullString.isEmpty || !seen.add(tag.fullString)) continue;
      tags.add(tag);
    }
    return tags;
  }
