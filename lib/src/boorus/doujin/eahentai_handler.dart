...
class EaHentaiHandler extends BooruHandler {
  ...
  /// Listing cards are plain `/a/{id}` links wrapping a cover image.
  @visibleForTesting
  List<BooruItem> itemsFromListing(String body) {
    ...
    for (final a in doc.querySelectorAll('a[href^="/a/"]')) {
      ...
      final String thumb = img?.attributes['src'] ?? img?.attributes['data-src'] ?? '';
      final String title = (img?.attributes['alt'] ?? '').trim();
