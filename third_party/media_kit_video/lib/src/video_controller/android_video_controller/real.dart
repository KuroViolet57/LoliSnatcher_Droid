  ...
  AndroidVideoController._(
    ...
    videoParamsSubscription = player.stream.videoParams.listen(
      (event) => lock.synchronized(() async {
        ...
        // LoliSnatcher patch (r59): the app may ask for a smaller surface
        // than the video's own size. It is capped here, the one place the size
        // is set, so the surface itself is created once and the plugin's own
        // lifecycle (wid, widListener, vo) is untouched.
        final Size? capped =
            PlatformVideoController.androidSurfaceSizeCap?.call(width, height);
        final int surfaceWidth = capped?.width.round() ?? width;
        final int surfaceHeight = capped?.height.round() ?? height;

        final isZero = surfaceWidth == 0 || surfaceHeight == 0;
        final isSame = surfaceWidth == rect.value?.width.toInt() &&
            surfaceHeight == rect.value?.height.toInt();
        if (isZero || isSame) {
          return;
        }
        ...
        await _channel.invokeMethod(
          'VideoOutputManager.SetSurfaceSize',
          {
            ...
            'width': surfaceWidth.toString(),
            'height': surfaceHeight.toString(),
          },
        );
