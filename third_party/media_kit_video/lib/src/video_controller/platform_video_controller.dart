...
abstract class PlatformVideoController {
  ...
  /// LoliSnatcher patch (r59): asked for the size the Android surface should
  /// use, given the video's own size; the same size back means no cap. Null —
  /// the published package's behaviour — uses the video's own size, which for
  /// an 8K video costs hundreds of MB of buffers. See LOLISNATCHER_PATCH.md.
  static Size Function(int width, int height)? androidSurfaceSizeCap;
